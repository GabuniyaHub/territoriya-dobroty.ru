...
> После первого запуска в `backend/prisma/dev.db` будет создана база данных SQLite. Животные не создаются автоматически — добавляйте их через админ-панель.

### Резервное хранение данных

Все данные о животных хранятся в файле базы данных `backend/prisma/dev.db`. Если сайт упадёт, информация о питомцах остаётся в этом файле. Чтобы не потерять данные, делайте резервные копии этого файла и храните их отдельно от сайта.

Например, можно создать копию перед обновлением или развернуть внешний бэкап:

```bash
cp backend/prisma/dev.db backend/prisma/dev.db.bak
```

Если планируется продакшн-развертывание, лучше использовать внешнюю СУБД (PostgreSQL, MySQL) и отдельно настраивать регулярные бэкапы.
